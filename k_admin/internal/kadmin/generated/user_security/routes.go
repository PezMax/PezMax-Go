...
// Register installs the CRUD routes for table ptmj_security and keeps the
...
func Register(api *gin.RouterGroup, deps Dependencies) error {
	...
	if err := ensurePermissions(deps.Connection); err != nil {
		return fmt.Errorf("sync user_security permissions: %w", err)
	}
	if err := ensureMenu(deps.Connection); err != nil {
		return fmt.Errorf("sync user_security menu: %w", err)
	}
	deps.RegisterAuditResource("user-security", "user_security", func(resourceID string) interface{} {
	...
	deps.RegisterIdempotentRoute("user-security")
	h := &handler{repo: repo}
	group := api.Group("/user-security", deps.RequireAuth)
...
// swaggerListUserSecurity documents GET /user-security.
// @Summary 查询用户密保列表
// @Tags 生成业务
// @Security BearerAuth
...
// @Router /user-security [get]
func (h *handler) list(c *gin.Context) {
	filter := UserSecurityFilter{
		Page:       positiveInt(c.Query("page"), 1),
		PageSize:   positiveInt(c.Query("pageSize"), 20),
		Keyword:    truncate(c.Query("keyword"), 100),
		Question:   truncate(c.Query("question"), 100),
		Answer:     truncate(c.Query("answer"), 100),
...
// swaggerGetUserSecurity documents GET /user-security/{id}.
// @Summary 获取用户密保详情
// @Tags 生成业务
// @Security BearerAuth
...
func (h *handler) get(c *gin.Context) {
	...
	if !found {
		httpx.Fail(c, http.StatusNotFound, errUserSecurityNotFound.Error())
		return
	}
	httpx.Success(c, item)
}

// swaggerCreateUserSecurity documents POST /user-security.
// @Summary 新增用户密保
// @Tags 生成业务
// @Security BearerAuth
// @Param payload body UserSecurityPayload true "用户密保信息"
...
// @Router /user-security [post]
func (h *handler) create(c *gin.Context) {
	var payload UserSecurityPayload
	if err := c.ShouldBindJSON(&payload); err != nil {
		httpx.Fail(c, http.StatusBadRequest, "invalid user_security payload")
		return
	}
	if message := validateUserSecurityPayload(payload); message != "" {
		httpx.Fail(c, http.StatusBadRequest, message)
		return
	}
	item, err := h.repo.create(payload)
	if err != nil {
		respondUserSecurityError(c, err)
		return
	}
	httpx.Success(c, item)
}

// swaggerUpdateUserSecurity documents PUT /user-security/{id}.
// @Summary 修改用户密保
// @Tags 生成业务
// @Security BearerAuth
// @Param id path int true "记录 ID"
...
// swaggerDeleteUserSecurity documents DELETE /user-security/{id}.
// @Summary 删除用户密保
// @Tags 生成业务
// @Security BearerAuth
// @Param id path int true "记录 ID"
...
// @Failure 403 {object} object
// @Failure 404 {object} object
// @Router /user-security/{id} [delete]
...
func validateUserSecurityPayload(payload UserSecurityPayload) string {
	if strings.TrimSpace(payload.Question) == "" {
		return "Question不能为空"
	}
	if strings.TrimSpace(payload.Answer) == "" {
		return "密保答案（BCrypt 密文，原样迁移）不能为空"
	}
	return ""
}

func respondUserSecurityError(c *gin.Context, err error) {
	switch {
	case errors.Is(err, errUserSecurityNotFound):
...
func ensurePermissions(conn db.Connection) error {
	seeds := []struct{ name, slug, method, path string }{
		{"用户密保查看", ListPermission, "GET", "/api/user-security*"},
		{"用户密保新增", CreatePermission, "POST", "/api/user-security*"},
		{"用户密保修改", UpdatePermission, "PUT", "/api/user-security*"},
		{"用户密保删除", DeletePermission, "DELETE", "/api/user-security*"},
	}
...
func ensureMenu(conn db.Connection) error {
	...
	rows, err := conn.Query(`SELECT id FROM goadmin_menu WHERE uri = ?`, "/user-security")
	if err != nil {
		return err
	}
	if len(rows) > 0 {
	...
	_, err = conn.Exec(`INSERT INTO goadmin_menu
		...
		parentID, "用户密保", "/user-security", "/kadmin/generated/user_security/UserSecurityListView")
	return err
}
