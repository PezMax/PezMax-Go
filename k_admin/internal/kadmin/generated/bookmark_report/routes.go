...
// Register installs the CRUD routes for table ptmj_bookmark_report and keeps the
...
func Register(api *gin.RouterGroup, deps Dependencies) error {
	...
	if err := ensurePermissions(deps.Connection); err != nil {
		return fmt.Errorf("sync bookmark_report permissions: %w", err)
	}
	if err := ensureMenu(deps.Connection); err != nil {
		return fmt.Errorf("sync bookmark_report menu: %w", err)
	}
	deps.RegisterAuditResource("bookmark-reports", "bookmark_report", func(resourceID string) interface{} {
	...
	deps.RegisterIdempotentRoute("bookmark-reports")
	h := &handler{repo: repo}
	group := api.Group("/bookmark-reports", deps.RequireAuth)
...
// swaggerListBookmarkReport documents GET /bookmark-reports.
// @Summary 查询书签举报列表
// @Tags 生成业务
// @Security BearerAuth
...
// @Router /bookmark-reports [get]
func (h *handler) list(c *gin.Context) {
	filter := BookmarkReportFilter{
...
// swaggerGetBookmarkReport documents GET /bookmark-reports/{id}.
// @Summary 获取书签举报详情
// @Tags 生成业务
// @Security BearerAuth
...
// @Router /bookmark-reports/{id} [get]
func (h *handler) get(c *gin.Context) {
	...
	if !found {
		httpx.Fail(c, http.StatusNotFound, errBookmarkReportNotFound.Error())
		return
	}
	httpx.Success(c, item)
}

// swaggerCreateBookmarkReport documents POST /bookmark-reports.
// @Summary 新增书签举报
// @Tags 生成业务
// @Security BearerAuth
// @Param payload body BookmarkReportPayload true "书签举报信息"
...
// @Router /bookmark-reports [post]
func (h *handler) create(c *gin.Context) {
	var payload BookmarkReportPayload
	if err := c.ShouldBindJSON(&payload); err != nil {
		httpx.Fail(c, http.StatusBadRequest, "invalid bookmark_report payload")
		return
	}
	if message := validateBookmarkReportPayload(payload); message != "" {
		httpx.Fail(c, http.StatusBadRequest, message)
		return
	}
	item, err := h.repo.create(payload)
	if err != nil {
		respondBookmarkReportError(c, err)
		return
	}
	httpx.Success(c, item)
}

// swaggerUpdateBookmarkReport documents PUT /bookmark-reports/{id}.
// @Summary 修改书签举报
// @Tags 生成业务
// @Security BearerAuth
// @Param id path int true "记录 ID"
...
// @Failure 403 {object} object
// @Failure 404 {object} object
// @Router /bookmark-reports/{id} [put]
...
// swaggerDeleteBookmarkReport documents DELETE /bookmark-reports/{id}.
// @Summary 删除书签举报
// @Tags 生成业务
// @Security BearerAuth
// @Param id path int true "记录 ID"
...
// @Failure 403 {object} object
// @Failure 404 {object} object
// @Router /bookmark-reports/{id} [delete]
...
func validateBookmarkReportPayload(payload BookmarkReportPayload) string {
...
func respondBookmarkReportError(c *gin.Context, err error) {
	switch {
	case errors.Is(err, errBookmarkReportNotFound):
...
func ensurePermissions(conn db.Connection) error {
	seeds := []struct{ name, slug, method, path string }{
		{"书签举报查看", ListPermission, "GET", "/api/bookmark-reports*"},
		{"书签举报新增", CreatePermission, "POST", "/api/bookmark-reports*"},
		{"书签举报修改", UpdatePermission, "PUT", "/api/bookmark-reports*"},
		{"书签举报删除", DeletePermission, "DELETE", "/api/bookmark-reports*"},
	}
...
func ensureMenu(conn db.Connection) error {
	...
	rows, err := conn.Query(`SELECT id FROM goadmin_menu WHERE uri = ?`, "/bookmark-reports")
	if err != nil {
		return err
	}
	if len(rows) > 0 {
	...
	_, err = conn.Exec(`INSERT INTO goadmin_menu
		...
		parentID, "书签举报", "/bookmark-reports", "/kadmin/generated/bookmark_report/BookmarkReportListView")
	return err
}
