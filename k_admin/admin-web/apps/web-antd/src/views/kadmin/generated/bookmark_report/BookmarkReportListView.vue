...
<template>
  <div class="page-stack">
    <section class="page-heading">
      <div>
        <h1>书签举报</h1>
        <p>代码生成器生成 · 表 ptmj_bookmark_report</p>
      </div>
      <a-space wrap>
    ...
    <section class="panel">
      <a-form :model="filters" layout="inline" class="search-form">
        ...
        <a-form-item label="审核结果：0-未审核，1-属实，2-不属实">
          <a-input
            v-model:value="filters.result"
            allow-clear
            class="control-md"
            placeholder="审核结果：0-未审核，1-属实，2-不属实"
            @press-enter="search"
          />
        </a-form-item>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'reportId'">
            <a-typography-text
              :content="record.reportId ?? '-'"
              :ellipsis="{ tooltip: record.reportId ?? '-' }"
            />
          </template>
          <template v-else-if="column.key === 'bookmarkId'">
            <a-typography-text
          ...
          <template v-else-if="column.key === 'result'">
            <a-typography-text
              ...
              :ellipsis="{ tooltip: record.result ?? '-' }"
            />
          </template>
    ...
    >
      <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
        <a-form-item name="bookmarkId" label="Bookmark Id">
          <a-input-number
            v-model:value="formState.bookmarkId"
            style="width: 100%"
            placeholder="请输入Bookmark Id"
            @change="markDirty"
          />
        </a-form-item>
        ...
        <a-form-item name="result" label="审核结果：0-未审核，1-属实，2-不属实">
          <a-input
            v-model:value="formState.result"
            placeholder="请输入审核结果：0-未审核，1-属实，2-不属实"
            @change="markDirty"
          />
        </a-form-item>
...
import {
  createBookmarkReport,
  deleteBookmarkReport,
  getBookmarkReportList,
  updateBookmarkReport,
  type BookmarkReport,
  type BookmarkReportPayload,
} from '#/api/kadmin/generated/bookmark_report';

type TablePagination = { current?: number; pageSize?: number };

const { hasAccessByCodes } = useAccess();
const canCreate = computed(() => hasAccessByCodes(['system:bookmark_report:create', '*']));
const canUpdate = computed(() => hasAccessByCodes(['system:bookmark_report:update', '*']));
const canDelete = computed(() => hasAccessByCodes(['system:bookmark_report:delete', '*']));

const loading = ref(false);
const errorText = ref('');
const items = ref<BookmarkReport[]>([]);
const total = ref(0);
const page = ref(1);
...
const columns = [
  ...
  { title: '审核结果：0-未审核，1-属实，2-不属实', dataIndex: 'result', key: 'result', width: 160, ellipsis: true },
...
async function fetchList() {
  ...
  errorText.value = '';
  try {
    const result = await getBookmarkReportList({
      page: page.value,
      pageSize: pageSize.value,
...
const rules: Record<string, unknown> = {
  bookmarkId: [{ required: true, message: '请输入Bookmark Id', trigger: 'blur' }],
...
function openCreate() {
  ...
  dirty.value = false;
  formState.bookmarkId = undefined;
...
async function openEdit(record: BookmarkReport) {
  editingId.value = record.reportId;
  dirty.value = false;
  formState.bookmarkId = record.bookmarkId;
...
async function submit() {
  ...
  try {
    const payload = {
      bookmarkId: formState.bookmarkId,
    ...
    } as BookmarkReportPayload;
    if (editingId.value === null) {
      await createBookmarkReport(payload);
      message.success('新增成功');
    } else {
      await updateBookmarkReport(editingId.value, payload);
...
async function remove(record: BookmarkReport) {
  try {
    await deleteBookmarkReport(record.reportId);
